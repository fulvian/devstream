...
const StoreMemoryInputSchema = z.object({
  ...
  source: z.string().optional().default('mcp_user_input')
});
...
export class MemoryTools {
    ...
    try {
      ...
      await this.database.execute(`
        INSERT INTO semantic_memory (
          id, content, content_type, content_format, keywords, source,
        ...
        content: [
          {
            ...
            text: `✅ **Memory Stored Successfully**\n\n` +
                  ...
                  `The information has been stored in DevStream semantic memory and can be retrieved using search queries.`
          }
        ]
      };
  ...
  async searchMemory(args: any) {
    try {
      ...
      // Build search query using SQLite FTS if available, otherwise use LIKE
      let sql = `
        SELECT
          sm.*,
          CASE
      ...
      sql += `
        ORDER BY relevance_score DESC, sm.importance_score DESC, sm.created_at DESC
        LIMIT ?
      `;
      ...
      await this.database.execute(
        `UPDATE semantic_memory SET access_count = access_count + 1, last_accessed = datetime('now') WHERE id IN (${memoryIds})`,
        [] // Empty params array
      );
      ...
      memories.forEach((memory, index) => {
        ...
        const relevanceText = memory.relevance_score >= 8 ? 'HIGH' : memory.relevance_score >= 5 ? 'MEDIUM' : 'LOW';

        output += `${index + 1}. ${typeEmoji} **${memory.content_type.toUpperCase()}** Memory\n`;
        output += `   📊 Relevance: ${relevanceText} (${memory.relevance_score}/10) • Importance: ${memory.importance_score.toFixed(2)}\n`;
        output += `   📍 Source: ${memory.source}\n`;

        // Parse and display keywords
        try {
          const keywords = JSON.parse(memory.keywords);
