...
from hooks.devstream.sessions.session_start import SessionStartHook
...
@pytest.fixture
async def session_start_hook(tmp_path: Path) -> SimplifiedSessionStartHook:
    ...
    original_project_root = simplified_module.project_root
    project_dir = tmp_path / "devstream-project"
    project_dir.mkdir()
    simplified_module.project_root = project_dir
    ...
    try:
        ...
        simplified_module.project_root = original_project_root
...
class TestSimplifiedSessionStartHook:
    ...
    async def test_error_handling_invalid_session(self, session_start_hook: SimplifiedSessionStartHook) -> None:
        ...
        with pytest.raises(ValueError):
            await session_start_hook._initialize_session("")
...
class TestSessionStartHookAdapter:
    """Tests for the SessionStartHook adapter used by Codex integration."""

    @pytest.mark.asyncio
    async def test_process_session_start_respects_cwd(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Ensure the adapter swaps project root and cleans up afterwards."""
        hook = SessionStartHook()

        # Avoid Direct DB calls and background tracking in unit test
        monkeypatch.setenv("DEVSTREAM_HOOK_SESSIONSTART", "false")

        async def _stub_start_tracking(self, session_id: str) -> None:  # type: ignore[override]
            self._logger.info("Stub tracking started", extra={"session_id": session_id})

        monkeypatch.setattr(
            "hooks.devstream.sessions.session_start_simplified.SessionTracker.start_tracking",
            _stub_start_tracking,
        )
        ...
        session_id = "test-adapter-session"

        results = await hook.process_session_start(
            {
        ...
        assert results["success"] is True
        assert (data_dir / "devstream.db").exists()
        assert simplified_module.project_root == original_root
        assert str(project_dir.resolve()) not in sys.path
