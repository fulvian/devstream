"""Unit tests for SessionTracker using AnyIO task groups."""
...
async def test_db(tmp_path: Path) -> str:
    ...
    db_path = tmp_path / "test_session_tracker.db"
...
async def session_tracker(session_manager: SessionManager) -> SessionTracker:
    ...
    return SessionTracker(session_manager)
...
class TestSessionTracker:
    ...
    @pytest.mark.asyncio
    async def test_start_tracking_completed_session(
        self, session_tracker: SessionTracker, session_manager: SessionManager
    ) -> None:
        """Test starting tracking for completed session raises exception."""
    ...
    @pytest.mark.asyncio
    async def test_cleanup_inactive_sessions(self, session_tracker: SessionTracker) -> None:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_tracking(self, session_tracker: SessionTracker, session_manager: SessionManager) -> None:
        """Test concurrent tracking of multiple sessions."""
        # Create multiple sessions
        sessions = []
        for i in range(3):
            session = await session_manager.create_session(f"concurrent-session-{i}")
            sessions.append(session)

        # Start tracking concurrently
        async with anyio.create_task_group() as tg:
            for session in sessions:
                tg.start_soon(session_tracker.start_tracking, session.id)

        # Verify all sessions are tracked
        assert len(session_tracker._active_trackers) == 3
        for session in sessions:
            assert session.id in session_tracker._active_trackers

        # Update progress concurrently
        async with anyio.create_task_group() as tg:
            for i, session in enumerate(sessions):
                tg.start_soon(
                    session_tracker.update_progress,
                    session.id,
                    tokens_used=(i + 1) * 10
                )
        ...
        # Verify updates
        for i, session in enumerate(sessions):
            metrics = await session_tracker.get_metrics(session.id)
            assert metrics.tokens_used == (i + 1) * 10

        # Stop tracking concurrently
        async with anyio.create_task_group() as tg:
            for session in sessions:
                tg.start_soon(session_tracker.stop_tracking, session.id)
    ...
    async def test_operation_time_tracking(self, session_tracker: SessionTracker, test_session: str) -> None:
        ...
        assert len(operation_times) == 10

        # Add more operations to test cleanup (keeps only last 100)
        for i in range(100):
            async with session_tracker.track_operation(test_session, f"bulk-operation-{i}"):
                pass

        # Should still have at most 100 operations
        assert len(session_tracker._operation_times[test_session]) <= 100

        # Cleanup
        await session_tracker.stop_tracking(test_session)

    @pytest.mark.asyncio
    async def test_session_health_monitoring(self, session_tracker: SessionTracker, test_session: str) -> None:
        """Test session health monitoring functionality."""
        await session_tracker.start_tracking(test_session)

        # Update some metrics
        await session_tracker.update_progress(test_session, tokens_used=50)

        # Wait a bit for health monitoring to run
        await asyncio.sleep(0.1)

        # Verify metrics are still being updated
        metrics = await session_tracker.get_metrics(test_session)
        assert metrics is not None
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_update_progress(self, session_tracker: SessionTracker) -> None:
        """Test error handling in update_progress with invalid session manager."""
        # Create tracker with invalid session manager
        class BrokenSessionManager:
            async def update_session(self, session_id: str, **kwargs):
                raise Exception("Database connection failed")

        broken_tracker = SessionTracker(BrokenSessionManager())

        # Should raise TrackingException
        with pytest.raises(TrackingException, match="Failed to update progress"):
            await broken_tracker.update_progress("test-session", tokens_used=10)